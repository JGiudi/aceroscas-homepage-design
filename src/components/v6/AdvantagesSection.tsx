...
import { Clock, Package, Users, Scissors, Truck } from "lucide-react";
...
const advantages = [
  ...
  {
    icon: Scissors,
    title: "Corte a medida sin costo",
    description:
      "Cortamos el material a la medida exacta que necesitás, sin cargo adicional. Tu producción arranca directo.",
    accent: "04",
  },
  {
    icon: Truck,
    title: "Entrega sin mínimo de venta",
    description:
      "No importa el tamaño de tu pedido. Respetamos todas las industrias — pequeñas, medianas y grandes.",
    accent: "05",
  },
];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-24">
          ...
          >
            <p className="font-body text-base text-background/40 max-w-sm leading-relaxed">
              Cada detalle de nuestro proceso está pensado para que la producción
              de tu fábrica no se detenga. La industria no espera, nosotros tampoco.
            </p>
          </motion.div>
        </div>

        {/* Cards — grid con gap-px como separador visual */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-background/10 border-t border-background/10">
          {advantages.map((item, i) => (
            <motion.div
              key={item.accent}
              className="group relative bg-foreground p-10 md:p-12 lg:p-14"
            ...
            >
              {/* Number */}
              <span className="font-display text-[6rem] md:text-[8rem] font-800 leading-none text-background/[0.03] absolute top-4 right-8 select-none group-hover:text-primary/10 transition-colors duration-700">
                {item.accent}
              </span>
              ...
              <p className="font-body text-background/40 leading-relaxed text-sm md:text-base">
                {item.description}
              </p>
